...
function HeroHeader() {
    const { t } = useI18n();
    return (<header className="bg-hero text-hero-foreground">
      <div className="mx-auto max-w-7xl px-5 pt-6 pb-16 sm:px-8">
        ...
        <div className="grid gap-12 md:grid-cols-[1.05fr_1fr] md:items-center md:gap-16">
          ...
          <div className="relative">
            <img src={heroImg} alt={t("home.hero.imgAlt")} width={1440} height={1152} className="aspect-[5/4] w-full rounded-4xl object-cover"/>
            ...
            <Mark name="Asterisk01" className="pointer-events-none absolute -right-4 -top-7 h-24 w-24 text-mark-yellow sm:-right-6 sm:-top-9 sm:h-28 sm:w-28"/>
          </div>
        </div>
      </div>
    </header>);
...
function Audiences() {
    const { tList } = useI18n();
    const audiences = tList("home.audiences");
    const targets = [
    ...
    return (<section id="find-a-coach" className="mx-auto -mt-10 max-w-7xl px-5 sm:px-8">
      <div className="grid gap-px overflow-hidden rounded-3xl border border-border bg-border md:grid-cols-2 lg:grid-cols-4">
        {audiences.map((a, i) => {
            ...
            if (isExternal[i]) {
                return (<a key={a.title + a.eyebrow} href={targets[i]} target="_blank" rel="noopener noreferrer" className={cardClassName}>
                {children}
              </a>);
            }
            return (<LocaleLink key={a.title + a.eyebrow} to={targets[i] ?? "/about"} className={cardClassName}>
              {children}
            </LocaleLink>);
        })}
      </div>
...
function WhyCredentialed() {
    ...
    const pillars = tList("home.pillars.items");
    return (<section className="relative mt-20 overflow-hidden bg-card py-24">
      <Mark name="CircularMark01" className="pointer-events-none absolute -right-20 top-10 h-64 w-64 text-mark-blue opacity-10"/>
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {pillars.map((p, i) => (<div key={p.title} className={cn("relative overflow-hidden rounded-3xl bg-primary p-8 text-primary-foreground", i % 2 === 1 ? "lg:translate-y-10" : undefined)}>
              <span aria-hidden="true" className="relative inline-grid h-16 w-16 place-items-center">
                <Mark name="CircularMark02" className="absolute inset-0 h-full w-full text-mark-blue"/>
...
function CoachingInAction() {
    const { t, tList } = useI18n();
    const themes = tList("home.insights.themes").map((item, i) => ({
        ...item,
        ...THEME_STYLES[i],
    }));
    return (<section className="bg-background py-24">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <LocaleLink to="/insights" className="text-sm font-semibold text-primary underline-offset-4 hover:underline">
            {t("home.insights.cta")}
          </LocaleLink>
        </div>
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {themes.map((th) => (<LocaleLink key={th.tag} to="/insights" className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-colors hover:border-chip-active-border">
...
function ForOrganisations() {
    const { t } = useI18n();
    return (<section id="organisations" className="bg-card text-foreground">
      <div className="mx-auto grid max-w-7xl gap-14 px-5 sm:px-8 py-24 md:grid-cols-2 md:items-center">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button asChild variant="pill-ghost" size="pill">
              <a href="https://coachingfederation.org/resources/resource-library/?_topic=coaching-in-organizations&_resource_type=case-studies" target="_blank" rel="noopener noreferrer">
        ...
        <div className="relative">
          <img src={leadershipImg} alt={t("home.organisations.imgAlt")} width={1600} height={1280} loading="lazy" className="aspect-[5/4] w-full rounded-2xl object-cover"/>
          <AiBadge label={t("common.aiGenerated")} className="absolute bottom-4 left-4"/>
        </div>
      </div>
    </section>);
...
function Sponsors() {
    ...
    const items = tList("home.ads.items").map((item, i) => ({
        ...item,
        image: AD_IMAGES[i],
    }));
    return (<section aria-label={t("home.ads.eyebrow")} className="border-t border-border bg-card py-24">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="max-w-2xl">
      ...
      <div className="mt-12">
        <SponsorMarquee items={items} adLabel={t("home.ads.adLabel")} cta={t("home.ads.cta")} aiLabel={t("common.aiGenerated")}/>
      </div>
    </section>);
}
const EVENT_STYLES = [
...
function Events() {
    const { t, tList } = useI18n();
    const events = tList("home.events.items").map((item, i) => ({ ...item, ...EVENT_STYLES[i] }));
    return (<section className="bg-card py-24">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {events.map((e) => (<LocaleLink key={e.title} to="/events" className={"group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition hover:-translate-y-0.5 " +
                CARD_SHADOW}>
              ...
              <div className="flex flex-1 flex-col p-6">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2">
                  {e.tags.map((tag) => (<span key={tag} className="inline-flex items-center rounded-full border border-border bg-chip px-2.5 py-1 text-xs font-semibold text-chip-foreground">
                      {tag}
                    </span>))}
                </div>
              </div>
...
function Research() {
    const { t, tList } = useI18n();
    const partners = tList("home.research.partners");
    return (<section className="bg-background py-24">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 text-center">
        ...
        <div className="mt-14 grid grid-cols-2 gap-4 md:grid-cols-5">
          {partners.map((p) => (<div key={p} className={"grid h-20 place-items-center rounded-2xl border border-border bg-card text-sm font-semibold text-foreground/70 " +
                CARD_SHADOW}>
              {p}
            </div>))}
        </div>
...
function Join() {
    const { t } = useI18n();
    return (<section className="relative overflow-hidden bg-hero text-hero-foreground">
      <Mark name="CircularMark02" className="pointer-events-none absolute -right-16 -top-10 h-96 w-96 text-mark-cream opacity-40"/>
      <div className="mx-auto grid max-w-7xl gap-10 px-5 sm:px-8 py-24 md:grid-cols-[1.2fr_1fr] md:items-center">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Button asChild variant="inverse" size="pill">
              <a href="https://coachingfederation.org/about/icf-membership/individual-membership/" target="_blank" rel="noopener noreferrer">
        ...
        <div className="rounded-2xl border border-hero-foreground/15 bg-hero-foreground/5 p-6 backdrop-blur">
          ...
          <form className="mt-5 flex flex-col gap-2 sm:flex-row" onSubmit={(e) => e.preventDefault()}>
            ...
            <Input id="home-newsletter-email" name="email" autoComplete="email" type="email" required placeholder={t("home.join.emailPlaceholder")} className="w-full rounded-full border-hero-foreground/20 bg-hero-foreground/10 text-hero-foreground placeholder:text-hero-foreground/70"/>
