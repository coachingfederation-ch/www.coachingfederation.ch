...
export function ProfileEditorHeader({
  ...
  return (
    <header className="rounded-3xl bg-hero p-8 text-hero-foreground shadow-soft md:p-10">
        ...
        <div className="rounded-2xl border border-hero-foreground/20 bg-hero-foreground/10 p-4">
          ...
          <p className="mt-1 flex items-center gap-2 text-sm font-semibold">
            <span
              aria-hidden
              className={
                "inline-block h-2 w-2 rounded-full " + (published ? "bg-accent" : "bg-highlight")
              }
            />
...
/** Sticky in-page list of the profile's areas — desktop only, anchors below. */
...
export function ProfileGroup({
  ...
  return (
    <InProfileGroup.Provider value>
      <section
        id={id}
        className="scroll-mt-8 rounded-3xl border border-border bg-card p-6 shadow-soft md:p-8"
      >
