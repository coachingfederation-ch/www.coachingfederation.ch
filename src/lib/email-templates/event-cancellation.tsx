/**
 * React Email template for attendee cancellation notices (free and paid).
 * Exports: template. Registered in lib/email-templates/registry.ts.
 *
 * Shares the chrome of the registration confirmation on purpose: an attendee
 * should recognise the sender at a glance, and the two mails are read together.
 */
...
import { CANCELLATION_COPY } from "./event-cancellation-copy";
...
export interface EventCancellationProps {
  ...
  /** Refund outcome as decided by the server, never by the mail template. */
  refund?: "refunded" | "pending" | "none";
  organiserEmail?: string;
  /** Set on the chapter's own copy so staff can tell it apart at a glance. */
  staffCopy?: boolean;
}
...
const Email = ({
  ...
  const refundLine =
    refund === "refunded"
      ? fill(copy.refundIssued, { amount: amount ?? "" })
      : refund === "pending"
        ? fill(copy.refundPending, { amount: amount ?? "" })
        : copy.refundNone;
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={content}>
            {staffCopy ? <Text style={staffNotice}>{copy.copyNotice}</Text> : null}
            ...
            <Section style={panel}>
              ...
              <Text style={{ ...value, margin: "4px 0 0" }}>{copy.calendarNote}</Text>
            </Section>

            {paid ? (
              <>
                <Hr style={rule} />
                <Text style={sectionTitle}>{copy.refundTitle}</Text>
                <Text style={paragraph}>{refundLine}</Text>
                {refund === "refunded" ? (
                  <Text style={paragraph}>{copy.refundIssuedNote}</Text>
                ) : null}
                {amount ? <Row title={copy.amountLabel}>{amount}</Row> : null}
                {reference ? <Row title={copy.referenceLabel}>{reference}</Row> : null}
              </>
            ) : null}
...
export const template = {
  ...
  displayName: "Event registration cancellation",
  previewData: {
    locale: "en",
    ...
    eventsUrl: "https://new.coachingfederation.ch/events",
