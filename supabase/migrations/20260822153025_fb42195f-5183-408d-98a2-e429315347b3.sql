...
BEGIN
  ...
  IF TG_OP = 'INSERT' AND NOT trusted THEN
    ...
    -- Refund columns are server-owned but NOT NULL: reset them to their
    -- documented defaults, never to NULL (which fails the not-null constraint
    -- and made every public registration error out).
